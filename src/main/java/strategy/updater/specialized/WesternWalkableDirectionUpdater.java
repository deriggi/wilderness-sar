...
//            dxDy[1] = acceleration[1];
//
//            log.log(Level.WARNING, "expanded acceleration {0} {1} ", new Object[]{(int) acceleration[0], (int) acceleration[1]});
//
//            if (Math.abs(acceleration[0]) == 0 && Math.abs(acceleration[1]) == 0) {
...
//            }
    ...
    private float[] expand(float[] loc, Raster2D raster) {
        ...
        raster.getWesternCutoutCells(visibleCells2, (int) loc[0], (int) loc[1], 8);
